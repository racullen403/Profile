...
for change of 16, our algorithm would return 10, 5, 1 which is not optimal.
"""


# greedy algorithm, tries largest coins first, then  moves to smaller coins
...
How it works below:
    ...
    We continue in this way, building up the dictionary with all the optimum solutions for 
    every potential change amount, using previous values.
    
"""


def dp_change(amount, coins_used=None):
    # Setup
    coins = coins_used
    if coins is None:
        coins = [1, 2, 5, 8, 10]
    ...
    for change in range(1, amount+1):
        if change not in min_change.keys():
            potential_min_change = min_change[change - 1] + [1]
            for coin in coins:
                if coin <= change:
...
for change in range(1, 17):
    print("Making change for {}:".format(change), dp_change(change))
